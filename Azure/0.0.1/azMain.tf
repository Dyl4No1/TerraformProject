...
resource "azurerm_resource_group" "rg" {
  ...
  name     = "${random_pet.prefix.id}-iBeacon-rg"
}
...
resource "azurerm_subnet" "iBeacon-subnet" {
  name                 = "${random_pet.prefix.id}-iBeacon-subnet"
  resource_group_name  = azurerm_resource_group.rg.name
...
resource "azurerm_network_security_group" "iBeacon-nsg" {
  ...
  ## add rules for sql, other vms and storage
}
...
resource "azurerm_network_interface" "iBeacon-nic" {
  ...
  ip_configuration {
    name                          = "iBeacon-nic"
    subnet_id                     = azurerm_subnet.iBeacon-subnet.id
...
resource "azurerm_storage_account" "iBeacon-storacc" {
...
resource "azurerm_windows_virtual_machine" "main" {
  ...
  boot_diagnostics {
    storage_account_uri = azurerm_storage_account.iBeacon-storacc.primary_blob_endpoint
  }
}
